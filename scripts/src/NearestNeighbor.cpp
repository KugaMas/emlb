...
/* Nearest Neighbor */
edn::NearestNeighbor::NearestNeighbor(uint16_t sizeX, uint16_t sizeY, std::tuple<int, int, int, int, bool> params) : EventDenoisor(sizeX, sizeY) {
	std::tie(supporters, distL2, deltaT, refractoryT, usePolarity) = params;
...
py::array_t<bool> edn::NearestNeighbor::run(py::array_t<uint64_t> arrts, py::array_t<uint16_t> arrx, py::array_t<uint16_t> arry, py::array_t<bool> arrp) {
...
int edn::NearestNeighbor::calculateDensity(dv::Event& event) {
	int nCorrelated = 0;

    auto prev = memMatrix[event.x * sizeY + event.y].back();

    if (event.ts - prev.ts <= refractoryT) return 0;
